...
class SyntheticEngine:
    ...
    async def broadcast_updates(self, updates):
        print(f"Broadcasting {len(updates)} updates")
        for u in updates:
            m = u["machine"]
            # Broadcast Position
            pos_payload = {
            ...
            if state["cycle_count"] > 0 and state["is_truck"]:
                task = self.db.query(models.Task).filter(models.Task.machine_id == m.id, models.Task.status == "In Progress").first()
                if task:
                    task.completed_quantity = state["cycle_count"] * self.thresholds["max_payload_tons"]
